...
export default {
  ...
  unsubscribeBars: function (uid: any) {
    var subIndex = _subs.findIndex((e: any) => e.uid === uid);
    if (subIndex === -1) {
...
socket.on("m", (e: any) => {
  ...
  const sub = _subs.find((e: any) => e.channelString === channelString);
